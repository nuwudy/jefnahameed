        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($mediaItems as $media)
                <div class="group bg-white border border-stone-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between card-hover-lift">
                    ...
                    <div class="relative aspect-[4/3] bg-stone-900 overflow-hidden cursor-pointer" wire:click="openLightbox({{ $media->id }})">
                        @if($media->type === 'video' && !$media->thumbnail_path)
                            <video src="{{ $media->display_url }}#t=0.5" preload="metadata" muted playsinline class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"></video>
                        @else
                        ...
                        <div class="absolute top-3 left-3 flex items-center gap-1.5 z-10">
                            @if(in_array($media->type, ['video', 'embed']))
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-[#54321A]/90 border border-[#DFB254]/40 backdrop-blur-xs text-[#FAF6F0] flex items-center gap-1">
                        ...
                        <div class="absolute inset-0 bg-stone-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-white z-10">
                            <span class="w-12 h-12 rounded-full bg-[#54321A]/80 border border-[#DFB254]/50 backdrop-blur-sm flex items-center justify-center shadow-lg">
                                @if(in_array($media->type, ['video', 'embed']))
                                    <svg class="w-5 h-5 fill-current text-[#DFB254] ml-0.5" viewBox="0 0 24 24">
                                        <path d="M8 5v14l11-7z"/>
                                    </svg>
                                @else
